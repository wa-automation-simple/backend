...
from middleware.security import decode_token, TokenData, SECRET_KEY, ALGORITHM
...
async def log_requests(request: Request, call_next):
    ...
    print(
        f"[{datetime.now()}] {request.method} {request.url.path} - Status: {response.status_code} - Time: {process_time:.3f}s"
    )
